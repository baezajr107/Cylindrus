...
public class CubeTestRenderer implements GLSurfaceView.Renderer
{
    ...
    public void onDrawFrame(GL10 unused)
    {
        ...
        if(!isHardcore || CubeTestSurfaceView.endgame) {
            for (int i = 0; i < 15; i++) {
                for (int j = 0; j < 16; j++) {
                    ...
                    Matrix.translateM(mRotationMatrix, 0, 0f, -14f + heightoffset + depthOffset, 0f);
                    Matrix.setLookAtM(mViewMatrix, 0, 0, 6f + cameraOffset, -16, 0f, 0f, 0f, 0f, 1.0f, 0f);
        ...
        for(int i=99;i>=passive.length-1-completedRows;i--) {
            for (int j=0; j<16;j++){
                ...
                Matrix.translateM(mRotationMatrix, 0, 0f, 0f, 2.18f);
                Matrix.scaleM(mRotationMatrix, 0, 0.5f, 0.5f, 0.5f);
                Matrix.translateM(mRotationMatrix, 0, 0f, -16f-heightoffset+depthOffset, 0f);
                Matrix.setLookAtM(mViewMatrix, 0, 0, 6f+cameraOffset, -16, 0f, 0f, 0f, 0f, 1.0f, 0f);
                Matrix.perspectiveM(mProjectionMatrix, 0, 60f, 3.0f / 5.0f, 0.1f, 100f);
                Matrix.multiplyMM(mMVPMatrix, 0, mProjectionMatrix, 0, mViewMatrix, 0);
                Matrix.multiplyMM(scratch, 0, mMVPMatrix, 0, mRotationMatrix, 0);
